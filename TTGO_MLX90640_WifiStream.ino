...
#define NUM_SAMPLES 128 // Number of samples to average for breathing plot

float mlx90640To[768];
paramsMLX90640 mlx90640;
float breathingData[NUM_SAMPLES];
...
void updateThermalData() {
  ...
  // Calculate the average temperature of the center 5x5 pixel region
  float avgTemp = 0;
  int centerX = 16, centerY = 12; // Center coordinates of 32x24 array
  for (int y = centerY - 2; y <= centerY + 2; y++) {
    for (int x = centerX - 2; x <= centerX + 2; x++) {
      avgTemp += mlx90640To[y * 32 + x];
    }
  }
  avgTemp /= 25;

  // Store the average temperature in the breathing data array
  breathingData[currentSampleIndex] = avgTemp;
  ...
  // Plot the breathing data over the heatmap
  plotBreathingData();
}
...
void drawHeatmap(float *data, float minTemp, float maxTemp)
{
  for (int y = 0; y < 24; y++)
  {
    for (int x = 0; x < 32; x++)
    {
      ...
      tft.fillRect(x * 4, y * 4, 4, 4, color);  // Scaling up each cell for better visualization
    }
  }
}
...
// Function to plot the breathing data on the TFT display
void plotBreathingData()
{
  int plotHeight = tft.height() * 0.3; // 30% of the display height
  int plotWidth = tft.width();
  int plotX = 0;
  int plotY = tft.height() - plotHeight;

  // Clear the plot area
  tft.fillRect(plotX, plotY, plotWidth, plotHeight, TFT_BLACK);

  // Find the min and max values in the breathing data
  float minVal = breathingData[0], maxVal = breathingData[0];
  for (int i = 1; i < NUM_SAMPLES; i++) {
    if (breathingData[i] < minVal) minVal = breathingData[i];
    if (breathingData[i] > maxVal) maxVal = breathingData[i];
  }

  // Plot the breathing data
  for (int i = 1; i < NUM_SAMPLES; i++) {
    int x0 = map(i - 1, 0, NUM_SAMPLES - 1, plotX, plotX + plotWidth);
    int y0 = map(breathingData[i - 1], minVal, maxVal, plotY + plotHeight, plotY);
    int x1 = map(i, 0, NUM_SAMPLES - 1, plotX, plotX + plotWidth);
    int y1 = map(breathingData[i], minVal, maxVal, plotY + plotHeight, plotY);
